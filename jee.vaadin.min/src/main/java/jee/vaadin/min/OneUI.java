...
public class OneUI extends UI {
    ...
    private void doLogin() {
		...
		loginWindow.addCloseListener(closeEvent -> {
			...
			if (loginWindow.getLoginResult().getType()==LoginResultType.LOGIN) {
				userAccount = userAccountService.login(loginWindow.getLoginResult().getUsername(), loginWindow.getLoginResult().getPassword());
				if (userAccount==null) {
	...
	private void loadPage() {
        ...
        footerLayout.addComponent(new Label(String.format("You are logged as \"%s\"", userIdentity.getUserAccount().getUsername())));
